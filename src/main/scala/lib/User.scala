...
object UserJsonImplicits {
	...
	implicit val userJsonReads: JsonReads[User] = JsonReads(bs => JsonSuccess(JsonNode.parse(bs.utf8String)))
	implicit val userJsonWrites: JsonWrites[User] = JsonWrites(jsv => ByteString(JsonNode.stringify(jsv)))
