...
case class YesterConsumer (topics: List[String]) extends Closeable with Runnable {
    ...
    implicit val pReqRreader: Reads[ProgrammeRequestMessage] = ProgrammeRequestMessageJsonImplicits.programmeRequestMessageReads
        ...
        allProgs.onComplete {
            case (Success(progList)) => {
                ...
                val durForReview: List[Programme] = for {
                    curProg <- progList
                    if ((!curProg.isPreProgramme) && (Date(curProg.get().progComponent.nextReview) <= inSixMonth))
                } yield curProg
                println(s"due for review list $durForReview")

                val recentlyApproved: List[Programme] = for {
                    curProg1 <- progList
                    if ((!curProg.isPreProgramme) && (threeMonthsAgo <= Date(curProg1.get().progComponent.approvedOn)))
                } yield curProg1
